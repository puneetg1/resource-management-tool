...
#     allocationStatus: Optional[str] = None, # ✅ CHANGED
...
#             query["Countdown"] = {"$lte": 30}
#         elif expiringStatus == "0-30":
#             query["Countdown"] = {"$gte": 0, "$lte": 30}
#         elif expiringStatus == "31-60":
#             query["Countdown"] = {"$gte": 31, "$lte": 60}
#         elif expiringStatus == "61-90":
#             query["Countdown"] = {"$gte": 61, "$lte": 90}
...
# Helper to build the query dynamically and consistently
def build_query(
...
# ✅ FIX: Restored the full logic for the bulk import function
@router.post("/employees/bulk-import-file", response_model=dict)
async def bulk_import_employees_from_file(file: UploadFile = File(...)):
    try:
        if not file.filename.endswith('.json'):
        ...
        for record in employees_raw_data:
            ...
            if end_date_str and isinstance(end_date_str, str) and end_date_str.strip():
                try:
                    ...
                    processed_record["Resource End date"] = None # Handle invalid date format
            else:
                processed_record["Resource End date"] = None
...
async def get_dashboard_summary():
    try:
        results = await asyncio.gather(
            ...
            list_collection.count_documents({"Countdown": {"$lte": 30}}),
            ...
            list_collection.aggregate([{"$match": {"Countdown": {"$gte": 0, "$lte": 90}}}, {"$bucket": {"groupBy": "$Countdown", "boundaries": [0, 31, 61, 91], "default": "Other", "output": {"count": {"$sum": 1}}}}, {"$project": {"name": {"$switch": {"branches": [{"case": {"$eq": ["$_id", 0]}, "then": "0-30 Days"}, {"case": {"$eq": ["$_id", 31]}, "then": "31-60 Days"}, {"case": {"$eq": ["$_id", 61]}, "then": "61-90 Days"}]}}, "value": "$count", "_id": 0}}]).to_list(length=None),
            list_collection.aggregate([{"$match": {"Countdown": {"$lte": 30}}}, {"$sort": {"Countdown": 1}}, {"$limit": 5}, {"$project": {"id": {"$toString": "$_id"}, "name": {"$concat": ["$First name", " ", "$Last name"]}, "daysLeft": "$Countdown", "project": "$Project", "_id": 0}}]).to_list(length=None),
