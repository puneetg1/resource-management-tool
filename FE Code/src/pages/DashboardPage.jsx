...
// // This is the robust ChartCard for the large, main chart
// const ChartCard = ({ title, data, children }) => (
//   <div className="grid grid-rows-[auto_1fr] bg-white/80 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700/50 rounded-xl p-4 sm:p-6 shadow-sm min-h-[500px]">
//     <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex-shrink-0">{title}</h4>
//     <div className="relative w-full">
...
// // ✅ NEW: Create a smaller version of ChartCard
...
//           {/* ✅ Use the new SmallChartCard for this chart */}
...
//           {/* ✅ And also use SmallChartCard for this chart */}
...
//   // ✅ MODIFIED: This handler now stops the event from bubbling to the chart's parent
//   const handleBarClick = (data, stream, event) => {
//     event.stopPropagation(); // This is important!
//     if (data && data.project && stream) {
//       navigate(`/records?Project=${encodeURIComponent(data.project)}&Stream=${encodeURIComponent(stream)}`);
//     }
//   };

//   // ✅ NEW: This handler is for clicking the general chart area to filter by project only
//   const handleChartAreaClick = (data) => {
//     if (data && data.activeLabel) {
//       const project = data.activeLabel;
//       navigate(`/records?Project=${encodeURIComponent(project)}`);
//     }
//   };

//   // ✅ FIX: This handler for the expiring contracts chart is now correct
//  const handleExpiringContractsClick = (data) => {
//   // This check correctly ensures the payload array is not empty
//   if (!data || !data.activePayload || !data.activePayload.length) return;
//   const payload = data.activePayload[0].payload;
//   const name = payload.name;
//   let status = '';
//   if (name === '0-30 Days') status = '0-30';
//   if (name === '31-60 Days') status = '31-60';
//   if (name === '61-90 Days') status = '61-90';
//   if (status) {
//     navigate(`/records?expiringStatus=${status}`);
//   }
// };
//   return (
//     <Layout>
...
//         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
//           <KpiCard title="Total Headcount" value={dashboardData?.kpis?.totalHeadcount ?? 0} icon={<Users size={24} className="text-blue-600 dark:text-blue-400" />} color="bg-blue-100 dark:bg-blue-900/30" />
//           <KpiCard title="At-Risk Contracts" value={dashboardData?.kpis?.atRiskContracts ?? 0} icon={<AlertTriangle size={24} className="text-red-600 dark:text-red-400" />} color="bg-red-100 dark:bg-red-900/30" onClick={() => navigate('/records?expiringStatus=at-risk')} />
//           <KpiCard title="Partially Allocated" value={dashboardData?.kpis?.partiallyAllocated ?? 0} icon={<FileText size={24} className="text-amber-600 dark:text-amber-400" />} color="bg-amber-100 dark:bg-amber-900/30" onClick={() => navigate('/records?allocationStatus=partial')} />
//           <KpiCard title="Active Projects" value={dashboardData?.kpis?.activeProjects ?? 0} icon={<Briefcase size={24} className="text-emerald-600 dark:text-emerald-400" />} color="bg-emerald-100 dark:bg-emerald-900/30" />
//         </div>
...
//               <ChartCard title="Project Stream Distribution" data={dashboardData?.charts?.projectStreamDistribution ?? []}>
//                 {/* ✅ ADDED: onClick handler for the chart area and cursor style */}
...
//                   <Tooltip cursor={{ fill: 'rgba(148, 163, 184, 0.1)' }} wrapperClassName="!border-gray-300 dark:!border-gray-600 !bg-white/80 dark:!bg-gray-800/80 !backdrop-blur-sm !rounded-lg" />
//                   <Legend />
//                   {/* ✅ MODIFIED: Pass the event object to the bar's onClick handler */}
...
//           <SmallChartCard title="Headcount by Stream" data={dashboardData?.charts?.headcountByStream ?? []}>
//             <PieChart>
//               <Pie data={dashboardData?.charts?.headcountByStream ?? []} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius="80%" labelLine={false} label={<CustomPieLabel />} onClick={(data) => navigate(`/records?Stream=${encodeURIComponent(data.name)}`)}>
...
//           {/* ✅ FIXED: Added cursor style to the BarChart wrapper */}
//           <SmallChartCard title="Contracts Expiring Soon" data={dashboardData?.charts?.expiringContractsBreakdown ?? []}>
//             <BarChart data={dashboardData?.charts?.expiringContractsBreakdown ?? []} margin={{ top: 20, right: 30, left: 0, bottom: 5 }} onClick={handleExpiringContractsClick} style={{ cursor: 'pointer' }}>
//               <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} />
//               <XAxis dataKey="name" stroke="#94a3b8" />
//               <YAxis stroke="#94a3b8" allowDecimals={false} />
//               <Tooltip cursor={{ fill: 'rgba(148, 163, 184, 0.1)' }} wrapperClassName="!border-gray-300 dark:!border-gray-600 !bg-white/80 dark:!bg-gray-800/80 !backdrop-blur-sm !rounded-lg" />
//               <Bar dataKey="value" name="Contracts" radius={[4, 4, 0, 0]}>
//                 {(dashboardData?.charts?.expiringContractsBreakdown ?? []).map((entry) => (<Cell key={`cell-${entry.name}`} fill={entry.name === '0-30 Days' ? '#ef4444' : '#f97316'} />))}
//               </Bar>
//             </BarChart>
//           </SmallChartCard>
...
//                 ))
//               ) : (
//                 <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400"><p>No at-risk employees.</p></div>
//               )}
//             </div>
//           </div>
//         </div>
...
//   // ✅ CHANGE: Colors reverted to the original bright palette
//   const CHART_COLORS = ['#3b82f6', '#10b981', '#f97316', '#ef4444', '#8b5cf6'];
...
//   // ✅ FIX: This is a new, more reliable handler for the expiring contracts chart
...
//           <SmallChartCard title="Contracts Expiring Soon" data={dashboardData?.charts?.expiringContractsBreakdown ?? []}>
//             {/* ✅ FIX: onClick is removed from the BarChart here */}
//             <BarChart data={dashboardData?.charts?.expiringContractsBreakdown ?? []} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
//               <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} />
//               <XAxis dataKey="name" stroke="#94a3b8" />
//               <YAxis stroke="#94a3b8" allowDecimals={false} />
//               <Tooltip cursor={{ fill: 'rgba(148, 163, 184, 0.1)' }} wrapperClassName="!border-gray-300 dark:!border-gray-600 !bg-white/80 dark:!bg-gray-800/80 !backdrop-blur-sm !rounded-lg" />
//               <Bar dataKey="value" name="Contracts" radius={[4, 4, 0, 0]}>
//                 {/* ✅ FIX: onClick is now on the Cell, making it reliable */}
//                 {(dashboardData?.charts?.expiringContractsBreakdown ?? []).map((entry) => (
//                   <Cell 
//                     key={`cell-${entry.name}`} 
//                     fill={entry.name === '0-30 Days' ? '#ef4444' : '#f97316'} 
